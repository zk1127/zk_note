...
开源图网络包:[pytorch geometric](https://github.com/rusty1s/pytorch_geometric)
...
- $y=\sum_{k=0}^K \theta_kT_k(\hat{L})x$
...
清华开源图嵌入:[openNE](https://github.com/thunlp/OpenNE)
...
- 半监督问题, 图中部分节点知道标签,部分节点作为测试集
- 对最后一层$\hat{Y}=MLP(Z)=Softmax(ZW+b)$

#### 链路预测

- 链路预测跟推荐系统十分相关
- 预测两个节点间是否存在边,比较麻烦的是数据只能确定哪些节点确定有边,不能真正确定无边
  - 采用近似的方式,负采样一些不存在的边作为负样本
...
- 文本分类
  - TextGCN:重构成了一个单词-文档图,文档分类问题转变为节点分类问题
- 语义角色标注
  - 将句子的词连接起来,构造一个有向的语法图,语义角色标注变成语法图的分类问题

#### PinSAGE

- GraphSAGE工业实现,根据随机游走权重采样
